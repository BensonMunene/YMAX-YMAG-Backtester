...
def backtest_strategy_4(df, asset="YMAX", initial_investment=10_000, zones_params=None):
    """
    Strategy 4: 
      Go long if VIX and VVIX fall into any one of the specified zones.
    """
    temp_df = df.copy()
    temp_df.sort_values("Date", inplace=True)
    temp_df.reset_index(drop=True, inplace=True)
    ...
    def in_any_zone(vix, vvix):
        if zones_params is None:
            return False
        for zone in zones_params:
            if (zone["vix_lower"] is not None and zone["vix_upper"] is not None and
                zone["vvix_lower"] is not None and zone["vvix_upper"] is not None):
                if zone["vix_lower"] <= vix <= zone["vix_upper"] and zone["vvix_lower"] <= vvix <= zone["vvix_upper"]:
                    return True
        return False
...
def calculate_performance_metrics(portfolio_df):
    ...
    return {
        ...
        "CAGR (%)": cagr,  # We'll remove or replace with a year-based version
...
**Strategy 4:** Allows you to specify multiple zones for VIX and VVIX. If the current VIX and VVIX 
values fall into any one of the defined zones then the strategy goes long. 
        """
    )
...
Your selections will determine the logic used in the subsequent backtest calculations.
    ...
    if strategy_choice == "Strategy 4":
        ...
        zones_count = st.number_input("Number of Zones in VIX and VVIX to go long:", min_value=1, max_value=5, value=2, step=1, on_change=reset_backtest)
        zones_params = []
        for i in range(1, int(zones_count) + 1):
            ...
            if i == 1:
                ...
                with col1:
                    zone_vix_lower = st.number_input("VIX Lower", value=default_vix_lower, key=f"vix_lower_zone_{i}", on_change=reset_backtest)
                with col2:
                    zone_vix_upper = st.number_input("VIX Upper", value=default_vix_upper, key=f"vix_upper_zone_{i}", on_change=reset_backtest)
                with col3:
                    zone_vvix_lower = st.number_input("VVIX Lower", value=default_vvix_lower, key=f"vvix_lower_zone_{i}", on_change=reset_backtest)
                with col4:
                    zone_vvix_upper = st.number_input("VVIX Upper", value=default_vvix_upper, key=f"vvix_upper_zone_{i}", on_change=reset_backtest)
            elif i == 2:
                ...
                with col1:
                    zone_vix_lower = st.number_input("VIX Lower", value=0, key=f"vix_lower_zone_{i}", on_change=reset_backtest)
                with col2:
                    zone_vix_upper = st.number_input("VIX Upper", value=0, key=f"vix_upper_zone_{i}", on_change=reset_backtest)
                with col3:
                    zone_vvix_lower = st.number_input("VVIX Lower", value=0, key=f"vvix_lower_zone_{i}", on_change=reset_backtest)
                with col4:
                    zone_vvix_upper = st.number_input("VVIX Upper", value=0, key=f"vvix_upper_zone_{i}", on_change=reset_backtest)
    ...
    if st.session_state["backtest_done"]:
        ...
        if st.session_state["ymag_df_final"] is not None:
            ...
            with c2:
                ...
                st.plotly_chart(fig_entry_exit_ymag, use_container_width=True)
