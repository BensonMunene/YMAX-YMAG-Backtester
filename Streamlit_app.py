...
Your selections will determine the logic used in the subsequent backtest calculations.
    ...
    if strategy_choice == "Strategy 4":
        ...
        zones_count = st.number_input("Number of Zones in VIX and VVIX to go long:", min_value=1, max_value=5, value=2, step=1)
        zones_params = []
        for i in range(1, int(zones_count) + 1):
            ...
            if i == 1:
                ...
                with col1:
                    zone_vix_lower = st.number_input("VIX Lower", value=default_vix_lower, key=f"vix_lower_zone_{i}")
                with col2:
                    zone_vix_upper = st.number_input("VIX Upper", value=default_vix_upper, key=f"vix_upper_zone_{i}")
                with col3:
                    zone_vvix_lower = st.number_input("VVIX Lower", value=default_vvix_lower, key=f"vvix_lower_zone_{i}")
                with col4:
                    zone_vvix_upper = st.number_input("VVIX Upper", value=default_vvix_upper, key=f"vvix_upper_zone_{i}")
            elif i == 2:
                ...
                with col1:
                    zone_vix_lower = st.number_input("VIX Lower", value=0, key=f"vix_lower_zone_{i}")
                with col2:
                    zone_vix_upper = st.number_input("VIX Upper", value=0, key=f"vix_upper_zone_{i}")
                with col3:
                    zone_vvix_lower = st.number_input("VVIX Lower", value=0, key=f"vvix_lower_zone_{i}")
                with col4:
                    zone_vvix_upper = st.number_input("VVIX Upper", value=0, key=f"vvix_upper_zone_{i}")
