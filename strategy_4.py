...
# ==============================================================================
# New Page Function: Ratio-Zones Backtest (Strategy 4)
# ==============================================================================
def display_ratios_zones_backtest():
    ...
    **Instructions**:
    ...
    # ------------------------------------
    # 2. Backtest Strategy 4 Function
    # ------------------------------------
    def backtest_strategy_4(
        ...
        ma_long_window=None,
        exit_on_filter_flip=True  # if True, exit when filter flips even if ratio remains in zone
    ):
        temp_df = df.copy()
        ...
        # Compute the raw ratio (VVIX/VIX) to 4 decimal places
        temp_df["VVIX/VIX Ratio"] = (temp_df["VVIX"] / temp_df["VIX"]).round(4)

        # For variant 4.2: compute moving averages on asset price
        if variant == "4.2" and ma_short_window and ma_long_window:
            ...
            def get_filter(ma_s, ma_l):
                if ma_s > ma_l:
                    ...
                    return "Neutral"
            temp_df["Filter_Signal"] = temp_df.apply(lambda row: get_filter(row["MA_short"], row["MA_long"]), axis=1)
        
        # Determine the ratio-based signal
        ratio_signals = []
        ...
        # Combine with filter for variant 4.2
        final_signals = []
        for i, row in temp_df.iterrows():
            ratio_signal = row["Ratio_Signal"]
            if variant == "4.1":
                final_signals.append(ratio_signal)
            else:
                filter_signal = row.get("Filter_Signal", "Neutral")
                if i == 0:
                    final_signals.append("No Investment")
                else:
                    prev_final = final_signals[i-1]
                    if ratio_signal == "Long":
                        if exit_on_filter_flip:
                            final_signals.append("Long" if filter_signal == "Bullish" else "No Investment")
                        else:
                            final_signals.append("Long" if (prev_final == "Long" or filter_signal == "Bullish") else "No Investment")
                    elif ratio_signal == "Short":
                        if exit_on_filter_flip:
                            final_signals.append("Short" if filter_signal == "Bearish" else "No Investment")
                        else:
                            final_signals.append("Short" if (prev_final == "Short" or filter_signal == "Bearish") else "No Investment")
                    else:
                        final_signals.append("No Investment")
        ...
        for i in range(1, len(temp_df)):
            ...
            if current_desired == "Long":
                if prev_strategy != "Long":
                    ...
                    temp_df.loc[i, "Strategy"] = "Long"
            elif current_desired == "Short" and asset in ["QQQ", "GLD"]:
        ...
        # Insert "VVIX/VIX Ratio" after "VVIX" if available
        if "VVIX" in temp_df.columns and "VVIX/VIX Ratio" in temp_df.columns:
            idx_vvix = temp_df.columns.get_loc("VVIX")
            ratio_series = temp_df.pop("VVIX/VIX Ratio")
            temp_df.insert(idx_vvix + 1, "VVIX/VIX Ratio", ratio_series)

        return temp_df

    # -----------------------------------------------
    # 4. Performance and Plotting Functions (Unchanged)
    # -----------------------------------------------
    ...
    def plot_portfolio_value(df, asset_label="Portfolio Value"):
        fig = px.line(
            ...
            x="Date",
            y="Portfolio_Value",
            title=f"Ratios Strategy - Portfolio Growth Over Time for {asset_label}",
    ...
    def plot_entry_exit(df, asset_name="YMAX"):
        ...
        # Portfolio Value line (secondary axis)
        fig.add_trace(
            go.Scatter(
                x=temp_df["Date"],
        ...
        # If MA filter is used, plot the moving averages
    ...
    ################################################################################
    # 5. Main UI Inputs for Ratio-Zones Backtest
    ################################################################################
    # Layout for Asset, Date, Frequency, and Sub-Strategy
    ...
    # Load Data using data_dir
    filename = get_filename(asset_option, timeframe_option)
    file_path = os.path.join(data_dir, filename)
    try:
        df_raw = load_data(file_path)
    except Exception as e:
        st.error(f"Could not load data for {asset_option} / {timeframe_option}: {e}")
        st.stop()
    ...
    if sub_strategy.startswith("4.2"):
        ...
        exit_on_filter_flip = True if exit_filter == "Yes" else False
    else:
        ma_short_window = None
        ma_long_window = None
    ...
    if run_btn:
        ...
        show_df = res_df.copy()
        show_df["Portfolio_Return"] = show_df["Portfolio_Return"] * 100
        show_df.rename(columns={"Portfolio_Return": "Portfolio_Return (%)"}, inplace=True)
...
# End of display_ratios_zones_backtest() function
