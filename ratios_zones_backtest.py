...
def display_ratios_zones_backtest():
    ...
    **Instructions**:
    ...
    def get_filename(asset, timeframe):
        ...
        elif asset in ["QQQ", "GLD", "TLT", "LQD", "QYLD"]:
            return f"{asset}_VIX_VVIX_{tf_suffix}.csv"
    ...
    def backtest_strategy_4(
        ...
        for i, row in temp_df.iterrows():
            ...
            if variant == "4.1":
                ...
                if i == 0:
                    ...
                    elif ratio_sig == "Short" and asset in ["QQQ", "GLD", "TLT", "LQD", "QYLD"]:
                        if exit_on_filter_flip:
        ...
        for i in range(1, len(temp_df)):
            ...
            elif current_desired == "Short" and asset in ["QQQ", "GLD", "TLT", "LQD", "QYLD"]:
    ...
    with col_sel1:
        st.subheader("Asset")
        asset_option = st.radio("Select the asset:", ["YMAX", "YMAG", "QQQ", "GLD", "TLT", "LQD", "QYLD"], index=2, on_change=reset_zones_backtest)
    ...
    with col_sel3:
        ...
        timeframe_option = st.selectbox("Data frequency:", ["Daily", "4H", "1H", "30M"], index=0, on_change=reset_zones_backtest)

    with col_sel4:
        st.subheader("Sub-Strategy")
        sub_strategy = st.radio("Choose Strategy", ["4.1 - Use Zones Only", "4.2 - Zones + MA Filter"], on_change=reset_zones_backtest)
    ...
    with col_sel2:
        from_date = from_date_placeholder.date_input(
            ...
            on_change=reset_zones_backtest
        )
    ...
    short_zones = []
    if asset_option in ["QQQ", "GLD", "TLT", "LQD", "QYLD"]:
        with col_zone2:
    ...
    if sub_strategy.startswith("4.2"):
        ...
        ma_short_window = st.number_input("Short MA Window", min_value=1, max_value=200, value=20, step=1, on_change=reset_zones_backtest)
        ma_long_window = st.number_input("Long MA Window", min_value=1, max_value=500, value=50, step=1, on_change=reset_zones_backtest)
        exit_filter = st.radio("Exit if filter flips (when ratio remains in zone)?", ["Yes", "No"], index=0, on_change=reset_zones_backtest)
    ...
    initial_cap = st.number_input("Initial Investment ($)", min_value=1000, max_value=1_000_000, value=10000, step=1000, on_change=reset_zones_backtest)
